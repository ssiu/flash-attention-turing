...
//     gmem_ptr[16b](0x2a78cd228200) o (_32,_128):(128,_1)
    ...
    // 16KB
    Tensor sdO = make_tensor(sV.data() + size(sV), SmemLayoutQ{});
    Tensor sdOt = make_tensor(sV.data() + size(sV), SmemLayoutQTransposed{});
...
//     }
    ...
    cudaFuncSetAttribute(compute_dq_kernel_v2, cudaFuncAttributeMaxDynamicSharedMemorySize, maxbytes);
    compute_dq_kernel_v2<<<dimGrid_dq, dimBlock_dq, maxbytes>>>(
                                                            q_ptr,
                                                            k_ptr,
                                                            v_ptr,
                                                            l_ptr,
