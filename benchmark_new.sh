    ...
    for hdim in "${hdims[@]}"; do
        for is_causal in "${is_causals[@]}"; do
            ...
            ncu --metrics gpu__time_duration.sum,sm__throughput.avg.pct_of_peak_sustained_elapsed \
                ...
                --csv python -c "import torch; from test_flash_attn_new import test_flash_attn_bwd; test_flash_attn_bwd(${batch_size}, ${num_heads}, ${num_heads_k}, ${seqlen}, ${seqlen}, ${hdim}, ${is_causal}, torch.float16)" \
