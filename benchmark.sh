    ...
    for hdim in "${hdims[@]}"; do
        for is_causal in "${is_causals[@]}"; do
            ...
            ncu --metrics gpu__time_duration.sum,sm__throughput.avg.pct_of_peak_sustained_elapsed \
                ...
                --csv python -c "import torch; from test_flash_attn import test_flash_attn; test_flash_attn(${batch_size}, ${num_heads}, ${num_heads_k}, ${seqlen}, ${seqlen}, ${hdim}, ${softmax_scale}, ${is_causal}, torch.float16)" \
